...
(() => {
  ...
  const TIER_LABEL = {
    T1:  "Confirmed senior",
    T2:  "Likely senior — strong match",
    T2b: "Likely senior — household pattern",
    T3:  "Recent grad — adjacent",
    T4:  "Possible senior — partial match",
    T5:  "Possible senior — list match only",
  };
  // First-word badge text for the table column.
  const TIER_BADGE = {
    T1: "Confirmed", T2: "Likely", T2b: "Likely",
    T3: "Adjacent", T4: "Possible", T5: "Possible",
  };
  ...
  const TIER_BANNER = {
    ...
    T4: {
      ...
      basisFn: r => `This household appears on a national list of parents of college-bound youth, and ${r.adult_42_63_count || "at least one"} parent-age adult is registered here. The second-adult or long-tenure signal is missing.`,
    },
    T5: {
      verdict: "A high-school senior may live here.",
      basisFn: () => `This household appears on a national list of parents of college-bound youth. No other signal corroborates it.`,
    },
  };

  // Glyph in the banner — color comes from the per-tier .banner--tX class.
  const TIER_GLYPH = { T1: "●", T2: "●", T2b: "○", T3: "◇", T4: "◐", T5: "○" };
  ...
  const TIER_CAVEAT = {
    ...
    T4: "Only one parent-age adult or short tenure — weaker than our two-adult signals.",
    T5: "Only the national-list match — no parent-age adults or long tenure to corroborate.",
  };
  ...
  function openDrawer(id) {
    ...
    // Drawer head: identity-only. Pin dot + owner name + address. The
    // adjacent-pill renders only for T3 (nothing for the senior tiers).
    ...
    $("#dTier").innerHTML = isAdjacent
      ...
      : "";
    $("#dName").innerHTML = `<span class="pin-preview ${tierKey} dName-dot"></span>${escape(r.display_name || "Unknown owner")}`;
    ...
    $("#drawerBody").innerHTML = `
      ...
      <div class="section-h">Owners on record</div>
