-- Tier + evidence-score computation, plus the v_targets view the frontend reads.

-- Recompute tier, evidence_score, evidence_chips, why_sentence for every
-- household. Idempotent. Run after any voter or Datazapp refresh.
...
begin
  ...
  -- Tier rule:
  --   T1: at least one voter age 17-18 at the address (anchor-relative)
  --   T2: no T1, but voter age 19-20 at the address
  --   T3: Datazapp hit + adult voter pattern (44-60 primary, two adults preferred), tenure 8+
  --   T4: Datazapp hit, profile fits younger cohort better than current senior
  --   T5: Datazapp-only, no adult voter corroboration
  --   TX: nothing — excluded from default views (parcels with no signal)
...
-- Public view: what the frontend reads.
create or replace view v_targets as
  select
    ...
    h.datazapp_hit, h.datazapp_match_count,
    h.tier,
    h.evidence_score,
    h.evidence_chips,
    h.why_sentence,
