-- T2 and T3 were inverted: recent-grad (kid already left) was outranking
-- likely-senior (probable current-senior parent missing from voter file).
-- Likely-senior is the higher-priority door, so it gets T2 and the brighter
-- pin treatment. Recent-grad drops to T3.
--
-- Default visible tiers tightens to T1+T2; T3 is off by default since
-- recent-grad parents are a softer signal.
